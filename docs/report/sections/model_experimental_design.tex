...
completed. All three candidate algorithms use the same raw development rows and
persisted fold assignments for a given dataset. Within each fold, preprocessing
is fit only on the fold-training rows and then applied to the validation rows,
so model differences are not confounded by different input representations or
validation-fold preprocessing statistics.
...
and preprocessing objects are re-created from scratch for every fold to prevent
fitted state or validation statistics from leaking between folds.
...
\centering
	...
	\begin{captionbelowboxaivn}
		\captable{Fixed model configurations used for the cross-dataset comparison.}
